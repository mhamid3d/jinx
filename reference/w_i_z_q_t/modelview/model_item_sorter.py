"""This module contains the ModelItemSorter, used by the
:class:`~wizqt.modelview.model.Model` class to perform in-memory sorting of
:class:`~wizqt.modelview.model_item.ModelItem` objects.
"""
...
class ModelItemSorter(QtCore.QObject):
    ...
    def __addSortColumn(self, column, direction):
        ...
        if column < 0 or not self._model.headerData(column, QtCore.Qt.Horizontal, wizqt.ROLE_IS_SORTABLE):
            return
    ...
    def defaultSortDirection(self, column):
        dataType = self._model.headerData(column, QtCore.Qt.Horizontal, wizqt.ROLE_TYPE)
    ...
    def sortItems(self, itemList):
        ...
        _LOGGER.log(dnlogging.VERBOSE,
                    "Sorting %s items on columns %s..." % (name, zip(self._sortColumns, self._sortDirections)))
        start = time.time()
